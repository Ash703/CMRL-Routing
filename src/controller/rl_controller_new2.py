...
TRAIN_BATCH_SIZE = 8
DEVICE = 'cpu'
FLOW_IDLE_TIMEOUT = 1              # seconds idle timeout for per-flow rules
GROUP_IDLE_TIMEOUT = 30            # seconds to keep unused groups before deletion
FLOW_HARD_TIMEOUT = 2
...
# s11: ports 1,2,3 connect to leaves 21,22,23 respectively
# s12: ports 1,2,3 connect to leaves 21,22,23 respectively (source_port numbers on spines)
SPINE_TO_LEAF_PORTS = {
    11: {21: 1, 22: 2, 23: 3},
    12: {21: 1, 22: 2, 23: 3},
}
...
class RLDCController(app_manager.RyuApp):
    ...
    def _packet_in(self, ev):
        ...
        chosen_idx = int(np.argmax(probs)) if not is_elephant else action_idx
        ...
        if not is_elephant:
            ...
            # elephant: install select group on ingress (weights proportional to probs)
            ...
            flow_type = 'elephant'
            self.logger.info("ELEPHANT %s->%s group %s ports %s weights %s at leaf %s", src, dst, gid, candidate_ports, weights.tolist(), ingress_leaf)
    ...
    def _is_elephant(self, flow_key, dpid):
        self.logger.info("Checking elephant for %s at dpid %s; flow_stats entries=%d", flow_key, dpid, len(self.flow_stats_cache.get(dpid, [])))
        stats = []
        with self.lock:
    ...
    # -------------------------
    def _install_simple_flow(self, datapath, src, dst, out_port, priority=200, idle_timeout=0, hard_timeout=5):
    ...
    def _install_flow_to_group(self, datapath, src, dst, group_id, priority=200, idle_timeout=0, hard_timeout=5):
        if datapath is None:
            return
        parser = datapath.ofproto_parser
        ofp = datapath.ofproto
